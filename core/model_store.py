...
def redirect_babeldoc_cache():
    ...
    try:
        ...
        _restore_babeldoc_offline_assets(target)
        return target
    except Exception as e:  # noqa: BLE001
...
def _restore_babeldoc_offline_assets(cache_dir):
    """Back-fill missing/corrupt BabelDOC assets from an offline package.

    Looks for babeldoc's own `assets/offline_assets_<tag>.zip` inside the cache
    (generated by tools/babeldoc_offline_assets.py; distributable as a single
    "PDF 字体包" file). When present, every missing font/cmap/tiktoken/layout-
    model file is restored from it with ZERO network — so a user who can't
    reach github/HF never stalls downloading fonts mid-translation. The zip's
    filename tag is version-bound: one generated for another babeldoc release
    simply won't match and is ignored (babeldoc would exit() on a mismatch).
    Best-effort: any failure just leaves BabelDOC to its normal downloads."""
    try:
        from pathlib import Path
        from babeldoc.assets import assets
        tag = assets.get_offline_assets_tag()
        zip_path = os.path.join(cache_dir, "assets", f"offline_assets_{tag}.zip")
        if not os.path.isfile(zip_path):
            return
        assets.restore_offline_assets_package(Path(zip_path))
        app_logger.info("BabelDOC assets verified/restored from offline package")
    except Exception as e:  # noqa: BLE001 — the offline pack is an optimization
        app_logger.warning(f"BabelDOC offline assets restore skipped: {e}")
