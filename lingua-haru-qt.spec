...
# GPU OCR: bundle the nvidia CUDA-12 runtime DLLs so onnxruntime-gpu's CUDA
# provider can load in the frozen OCR subprocess (no site-packages/nvidia/ then).
# -> nvidia_cuda/ at the bundle root; the OCR child adds it to the DLL search path
# (image_translation_pipeline._bundled_nvidia_dir). Empty for a CPU onnxruntime.
