...
3. 依存関係のインストール
    - 依存パッケージ
        ...
        ```
    - オプションモジュール（必要なものだけインストール、UIが自動的に有効化します）
        ```bash
        # 画像翻訳（.png/.jpg など）：OCR認識 + 訳文を画像に描画
        pip install -r plugins/ocr/requirements.txt

        # PDF 翻訳（.pdf）：BabelDOC でレイアウト保持
        pip install -r plugins/pdf/requirements.txt

        # 動画/音声字幕（.mp4/.mp3 など）：文字起こし（Whisper / SenseVoice / Qwen-ASR）後に翻訳
        # ffmpeg は同梱（imageio-ffmpeg）。PATH 不要。リアルタイム音声 + 任意の話者ラベルにも使用
        pip install -r plugins/video/requirements.txt

        # クイック翻訳の音声：マイク入力 + 読み上げ（edge-tts + 共有 STT）
        pip install -r plugins/speechio/requirements.txt
        # 注：任意モデル（OCR / STT / 話者分離 / PDF レイアウト）は初回使用時に data/models へ自動ダウンロード
        ```


4. ツールの実行

    **Web アプリ (FastAPI)** —— ブラウザ UI：
    ```bash
    ...
    **デスクトップアプリ (Qt)** —— 推奨のネイティブデスクトップ体験（Fluent Design）:
    ```bash
